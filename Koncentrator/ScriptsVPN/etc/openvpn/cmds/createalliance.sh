...
if [ $? -eq 0 ]; then fail=1;
  ...
  sudo iptables -A FORWARD -i $device -o $device -s $2 -d $3 -j ACCEPT
...
if [ $? -eq 0 ]; then fail=1;
  ...
  sudo iptables -A FORWARD -i $device -o $device -s $3 -d $2 -j ACCEPT
  if [ $? -ne 0 ]; then fail=1; fi
fi

sudo iptables -C FORWARD -i $device -o $device -s $2 -d $3 -j DROP
if [ $? -eq 0 ]; then fail=1;
  sudo iptables -D FORWARD -i $device -o $device -s $2 -d $3 -j DROP
  if [ $? -ne 0 ]; then fail=1; fi
fi

sudo iptables -C FORWARD -i $device -o $device -s $3 -d $2 -j DROP
if [ $? -eq 0 ]; then fail=1;
  sudo iptables -D FORWARD -i $device -o $device -s $3 -d $2 -j DROP
  if [ $? -ne 0 ]; then fail=1; fi
fi
